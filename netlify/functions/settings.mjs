...
// store:config -> { storeName, whatsapp, contactEmail, pointsPerQuetzal, pointsToQuetzalRate, updatedAt }
...
const DEFAULT_SETTINGS = {
  ...
  pointsPerQuetzal: 1,
  // Tasa de canje: cuántos puntos equivalen a Q1 de descuento al canjear.
  // Ej: 10 significa que 10 puntos = Q1 de descuento.
  pointsToQuetzalRate: 10,
  updatedAt: null
};
...
export default async (req) => {
  ...
  try {
    ...
    if (req.method === 'PUT' || req.method === 'POST') {
      ...
      let pointsToQuetzalRate = body.pointsToQuetzalRate !== undefined
        ? Number(body.pointsToQuetzalRate)
        : current.pointsToQuetzalRate;

      if (!Number.isFinite(pointsToQuetzalRate) || pointsToQuetzalRate <= 0) {
        pointsToQuetzalRate = DEFAULT_SETTINGS.pointsToQuetzalRate;
      }
