...
import { chargeCard } from './payment-gateway.mjs';

// Pedidos de Grupo Rolmar
// order:<id> -> {
//   id, email, customerName, guest, contactEmail, items, total, creditApplied, amountCharged,
//   transactionId, paymentStatus, pointsEarned, billing, address, status, trackingNumber, createdAt
// }
//   - Pedido de cliente con cuenta: email = correo de la cuenta, guest = false.
//   - Pedido de invitado (sin sesión): email = '', guest = true, contactEmail = correo opcional que dio.
//     No suma puntos ni usa saldo.
// orders:index -> [ids]
...
// ---------- Leer la tasa de puntos configurada (por defecto 1 punto por Q1) ----------
async function getPointsPerQuetzal() {
  try {
    const settings = await settingsStore().get('store:config', { type: 'json' });
    const rate = settings ? Number(settings.pointsPerQuetzal) : NaN;
...
// ---------- Normalizar los datos de facturación (CF por defecto, o NIT) ----------
function resolveBilling(rawBilling) {
  if (rawBilling && rawBilling.type === 'NIT') {
    const nit = String(rawBilling.nit || '').trim();
    if (!nit) return { error: 'Falta el NIT para la factura' };
    return {
      billing: {
        type: 'NIT',
        nit,
        name: rawBilling.name ? String(rawBilling.name).trim() : ''
      }
    };
  }
  return { billing: { type: 'CF' } };
}
...
async function notifyCustomer(email, payload) {
  ...
  if (!email) return; // pedidos de invitado no tienen cuenta ni notificaciones push
...
export default async (req) => {
  ...
  try {
    // ---------- GET: listar pedidos ----------
    // ?guestIds=a,b,c = pedidos de invitado guardados en el celular del cliente (solo devuelve pedidos de invitado).
    // ?token=         = solo los pedidos de ese cliente con cuenta.
    // Sin nada        = todos (admin).
    if (req.method === 'GET') {
      const guestIdsParam = url.searchParams.get('guestIds');
      if (guestIdsParam !== null) {
        const ids = guestIdsParam.split(',').map((x) => x.trim()).filter(Boolean).slice(0, 50);
        const guestOrders = [];
        for (const orderId of ids) {
          const order = await os.get(`order:${orderId}`, { type: 'json' });
          if (order && order.guest) guestOrders.push(order);
        }
        guestOrders.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
        return jsonResponse(200, guestOrders);
      }
    ...
    // ---------- POST: crear pedido ----------
    // Con token = cliente con cuenta (usa saldo, gana puntos, guarda dirección).
    // Sin token = invitado (paga el total, no gana puntos, no se guarda nada en cuentas).
    if (req.method === 'POST') {
      ...
      const card = body.card || null; // datos/token de tarjeta (la pasarela simulada no los valida)
      ...
      const { billing, error: billingError } = resolveBilling(body.billing);
      if (billingError) return jsonResponse(400, { error: billingError });

      // ---------- Quién compra ----------
      let user = null;
      let guestEmail = '';
      if (isGuest) {
        guestEmail = body.email ? String(body.email).trim().toLowerCase() : '';
        ...
        user = await as.get(`user:${session.email}`, { type: 'json' });
      ...
      // ---------- Aplicar saldo de puntos (creditBalance) como descuento automático (solo con cuenta) ----------
      const availableCredit = user ? (user.creditBalance || 0) : 0;
      const creditApplied = Math.round(Math.min(availableCredit, total) * 100) / 100;
      const amountToCharge = Math.round((total - creditApplied) * 100) / 100;

      // ---------- Cobrar la diferencia con la pasarela (simulada por ahora) ----------
      // Si el saldo cubre todo el pedido (amountToCharge === 0), no se llama a la pasarela.
      let transactionId = null;
      if (amountToCharge > 0) {
        const orderIdForCharge = crypto.randomUUID();
        const chargeResult = await chargeCard({
          amount: amountToCharge,
          email: user ? user.email : guestEmail,
          orderId: orderIdForCharge,
          card
        });

        if (!chargeResult.success) {
          return jsonResponse(402, { error: chargeResult.error || 'No se pudo procesar el pago' });
        }
        transactionId = chargeResult.transactionId;
      }

      // A partir de aquí el pago ya está aprobado (o no era necesario): se descuenta stock y se crea el pedido.
      ...
      // Los invitados no ganan puntos
      const pointsPerQuetzal = user ? await getPointsPerQuetzal() : 0;
      const pointsEarned = Math.floor(total * pointsPerQuetzal);

      const orderId = crypto.randomUUID();
      const order = {
        id: orderId,
        email: user ? user.email : '',
        customerName: user ? user.name : String(address.fullName).trim(),
        guest: isGuest,
        contactEmail: isGuest ? guestEmail : '',
      ...
      // ---------- Invitado: aquí termina ----------
      if (isGuest) {
        return jsonResponse(201, { order, guest: true });
      }
